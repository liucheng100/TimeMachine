...
<template>
    <div class="review-submissions" @scroll="setHeight()">
        ...
        <div class="all-works" :style="top_change">
            <WorkCard v-for="(item, index) in card_info" :ref="item.value.ref" :key="item.value.workId"
                :WorkName="item.value.workTitle" :contestGroup="item.value.contestGroup" :Status="item.value.isPass"
                :is_choose="item.value.is_choose" :ViewVolume="item.value.views" :is_multiply="item.value.is_multiply"
                :info="item.value" :src="item.value.coverFile"
